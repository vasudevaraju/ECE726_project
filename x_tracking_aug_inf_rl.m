...
%this RL algorithm for generating the optimal controlrefer to the following
%paper: https://doi.org/10.1109/TAC.2014.2317301
...
%xref = 10 for simplicity. The linearized model of the plant allows for 
...
Q= eye(1); %costs are defined for calculating the reward
R= 1; %These are to be picked by the designer before running the RL algo
Qaug = Caug'*Q*Caug;
S = place(A,B,-[2,5,4,9]); %Initial guess of the control gain S
...
K(5,5) = 0; 
%Augment a 5th row and column to which corresponds to the tajectory dynamics
sample_num = 40; %no of samples to be collected in one iteration
sample_time = 0.05; %sampling time
t0 = 0; %initial iteration time
x0 = [0.1,0.1,0.1,0.1,5]; 
...
while 1
    ...
    dummy_act = zeros(size(X,1),15);
    activation = zeros(size(X,1)-1,15);
...
%Though the control law was obtained for trajectory xref = 5, it also holds
%for any trajectory of the form xref = constant. Here xref =10 is plotted.
legend('x1','x2','x3','x4','ref');
title('Reinforcement learning state evolution');xlabel('t');ylabel('x');
[t,x]=ode45(@(t,x)(Aaug-Baug*inv(R)*Baug'*K_new)*x,[0,10],[1,0,0,0,10]);
